...
| Group 5 — Error handling | Retry, network handling, interruption resilience, per-file continuation | Workflow classifies translation failures into transient vs non-retryable input classes, retries transient errors once, and returns typed error responses (preflight 422, translation 502); pipeline is itemized per file for partial continuation. | Add durable run-state store and richer class-specific backoff policy. |
