...
| Group 2 — Parsing/Extraction | Preserve formulas/tables/layout; metadata fidelity | Delegated to PDFMathTranslate-next execution node, preserving layout through engine constraints; workflow now emits a post-run fidelity checklist summary with pass/warn verdict. | Upgrade checklist from size-based heuristic to parser/visual diff probes and confidence scoring. |
...
| Group 4 — Reconstruction/Output | Bilingual output, preserved styles/positions, downloadable artifact | Output path is deterministic and returned to caller; reconstruction performed by PDFMathTranslate-next; workflow enforces naming metadata convention (`runId-sourceStem-bilingual-v1`), verifies non-empty output PDF before success, and emits artifact metadata (`storageKey`, optional `downloadUrl`). | Add real S3/MinIO publish/upload step (currently metadata publication only). |
...
| Group 6 — n8n integration | Automated operation and observability | n8n JSON workflow provided; request/response pattern supports API integration; terminal responses now include per-file `audit` metadata (hash, command args, timing, status, error class) and optional notification webhook payloads for success/partial/preflight-failure/translation-failure outcomes sent through n8n HTTP Request nodes. | Add trace IDs, persistent audit sink, dashboard metrics export, and channel adapters beyond webhook. |
