...
| Group 4 — Reconstruction/Output | Bilingual output, preserved styles/positions, downloadable artifact | Output path is deterministic and returned to caller; reconstruction performed by PDFMathTranslate-next; workflow verifies a non-empty output PDF exists before returning success and emits artifact metadata (`storageKey`, optional `downloadUrl`). | Add real S3/MinIO publish/upload step (currently metadata publication only). |
